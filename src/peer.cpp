...
Peers::Peer::Peer (int fd, SSL * ssl, std::string user, int address,
                   networkT nl)
{
  _fd = fd;
  _ssl = ssl;
  ...
  Log::Debug2 ("Added fd %d to fd_set master (1st free fd: %d)", fd,
               db.size ());
...
bool Peers::Peer::operator>> (Network::Packet * packet)
{
  if (!
      (packet->length =
         xSSL_read (_ssl, packet->buffer, 4096, "forwarding data")))
    {
      _state = PEER_CLOSING;
      return FAIL;
    }
  ...
  Log::Debug3 ("Read %d bytes packet from peer %s", packet->length,
               _user.c_str ());
  return DONE;
}
...
{
  if (!xSSL_write
      (_ssl, packet->buffer, packet->length + 2, "forwarding data"))
    {
      _state = PEER_CLOSING;
      return FAIL;
    }
...
bool Peers::Peer::isRoutableAddress (int address)
{
  uInt i;
...
void Peers::FreeNonActive ()
{
  ...
  for (i = 0; i < db.size (); i++)
    if (!db[i]->isActive ())
      {
        Taps::setSystemRouting (db[i],
                                Taps::getUserAllowedNetworks (db[i]->user ()),
                                DEL_ROUTING);
        delete db[i];
...
void Peers::Peer::Disassociate ()
{
  ...
  Taps::setSystemRouting (this, Taps::getUserAllowedNetworks (_user),
                          DEL_ROUTING);
  delete this;
}
